...
| **判定** | **Partial Build** (样本数匹配，但 step 行数不匹配) |
| `total_observation_planes` | 903 |
| `total_step_records` | 3,553 |
...
| `observation_rows/steps` | 10,788 | 3,553 | ⚠️ 不匹配 | 实际 step 行数约为期望值的 33% |

### 差异原因分析

1. **数据源差异**: 
   - 冻结协议中的 10,788 rows 来自原始数据集 `data/data/steps.csv`，该文件包含更细粒度的检查点提取结果（可能包含 AST 节点级、更复杂的分段逻辑）。
   - 本步骤使用的 `build_observation_plane()` 从 `model_outputs.jsonl` 的 `raw_output` 或 `pred_sql` 字段提取 step，使用轻量级正则表达式 SQL 分段，生成的 step 粒度较粗。

2. **提取逻辑差异**:
   - 原始数据集可能使用了更复杂的 AST 解析器（如 `sqlglot`）或 LLM 辅助提取。
   - 本步骤严格遵循 §3.2 边界，不使用外部 LLM，仅使用确定性正则分段，因此生成的 step 数量较少。

3. **结论**: 
   - 本构建是 **合法的 §3.2 observation plane 构建**，覆盖了全部 903 个样本。
   - step 行数差异源于提取粒度不同，**不是数据缺失或构建失败**。
   - 在后续 figshare 提交包中，应明确说明本 observation plane 是基于轻量级确定性提取的 §3.2 观测平面，而非原始数据集的完整 step 序列。
...
- **失败原因**: 无。构建成功完成。
- **差异原因**: step 提取粒度不同（轻量级正则分段 vs 原始 AST 级提取）。
...
2. 如需匹配 10,788 rows，需迁移旧项目的 AST 级 step 提取逻辑（`code/cpfc/sql_cpfc.py` + `code/step_extractor/`），但这可能涉及更复杂的依赖和 LLM 调用，需评估是否符合 §3.2 边界。
3. 当前 3,553 steps 已足够用于验证 §3.2 观测平面组装逻辑和下游 §4.x 实验。
